...
public class routertest
{
	...
	public static Boolean debugnmr = true;
	...
	public void inputHandler(String line, int lan, String Filetype, String msgType) throws NumberFormatException, IOException
	{
		switch (msgType.toLowerCase()) 
		{
			...
			case "receiver":
			{
				...
				if(this.rTable[lan].pruneLoop == false)
				{
	...
	public void handleData(String line) throws IOException
	{
		...
		if(this.alldatastreams.containsKey(hostlan))
		{
			...
			{
				System.out.println("new data stream from another router added");
				this.alldatastreams.get(hostlan).add(incominglan);
			}
		}
		else
		{
			System.out.println("initial data stream from host lan recorded");
			this.alldatastreams.put(hostlan, new ArrayList<Integer>());
			this.alldatastreams.get(hostlan).add(incominglan);
			System.out.println("initial data stream from host lan recorded: " + hostlan + " " + this.alldatastreams.get(hostlan).toString());
		}
			...
			else
			{
				...
				if(this.alldatastreams.get(hostlan).size() == this.rTable[incominglan].attatchedRouters.size())
				{
					...
					if(this.rTable[incominglan].Receiver)
					{
						for(int x : this.rTable[incominglan].attatchedRouters.keySet())
						{
							...
							if(x < this.RouterID)
							{
								if(debugdata)
								{
									System.out.println("lower router id");
								}
								higherID = true;
							}
						}
						...
						if(higherID == false)
						{
							if(debugdata)
							{
								System.out.println("higher router id");
							}
							return;
						}
					}
					...
					if(this.rTable[incominglan].pruneLoop == false)
					{
						if(debugdata)
						{
							System.out.println("Router " + this.RouterID + " Pruning LAN " + incominglan);
						}
				...
				{
					System.out.println("Wtf " + this.alldatastreams.get(hostlan).size() + " " + this.rTable[incominglan].attatchedRouters.size());
				}
	...
	public void sendData(int incominglan, int hostlan) throws IOException
	{
		...
		for(int i : LANs.keySet())
		{
			...
			if(this.rTable[i].NMR == false)
			{
				...
				if(debugfileio || debugdata)
				{
					System.out.println("Router " + this.RouterID + " writing data to LAN " + i);
				}
			}
		}
		outputData.close();
	...
	public static void main(String[] args) throws IOException, InterruptedException
	{
		...
		routertest myRouter = new routertest();
